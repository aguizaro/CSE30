...
def print_board(chosen_letters_, hidden_word_, num_lives_, health_bar_):
    # format and print the game interface:
        ...
        chosen= ', '.join(chosen_letters_) if chosen_letters_ else None
        if chosen_letters_:
            board= f'Letters chosen: {chosen}\n{hidden_word_}    lives: {num_lives_}   {health_bar_}'
        else:
            board= f'Letters chosen: \n{hidden_word_}    lives: {num_lives_}   {health_bar_}'
    ...
    while (playagain):
        ...
        while (not game_won):
            ...
            if board: #only print the board if chose a valid letter
                print(print_board(chosen_letters, hidden_word, num_lives, health_bar))
            board= True
