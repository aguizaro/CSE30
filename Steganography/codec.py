'''
Codec.py by Antonio Guizar Orozco
Codec has 3 classes, Codec, CaesarCypher, and HuffmanCodes all used in Stegenography.py
'''
import numpy as np
...
class CaesarCypher(Codec):
    ...
    def encode(self, text):
        ...
        if type(text) == str:
            for i in text:
                if ord(i) == 32:
                    data+= ''.join([format(ord(' ')+self.shift, "08b")])
                else: 
                    data+= ''.join([format((ord(i)+self.shift) %256, "08b")])
        else:
            print('Format error')
            return
        return data
